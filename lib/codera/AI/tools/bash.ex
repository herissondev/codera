defmodule Codera.AI.Tools.Bash do
  ...
  * To run `go test ./...`: `{ cmd: "go test ./..." }`
  * To run `cargo build` in the `core/src` subdirectory: `{ cmd: "cargo build", cwd: "/home/user/projects/foo/core/src" }`
  * To run `ps aux | grep node`: `{ cmd: "ps aux | grep node" }`
  * To print a special character like `$` with some command `cmd`: `{ cmd: "cmd \$" }`
